...
mean = data["atr"].mean()
data["atr"] = data.apply(lambda x: x.atr * 1e3 if x.atr is not None else mean * 1e3, axis=1)
mean = data["cvr"].mean()
data["cvr"] = data.apply(lambda x: x.cvr * 1e3 if x.cvr is not None else mean * 1e3, axis=1)
...
data = data.drop(columns=['action', 'random', 'p_view', 'p_click', 'is_addtocart', 'is_order'])
...
if not os.path.exists('unice_rank_model'):
  ...
  print("load unice_rank_model ..........")
  model = tf.keras.models.load_model('unice_rank_model')
