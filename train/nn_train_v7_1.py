...
from feature_handle_v4 import features_extract, fea_inputs
...
parse.add_argument('--start_date', type=str, default='20220905')
...
# 正常训练记得关闭
parse.add_argument('--allow_cold_start', type=bool, default=False)
...
model_path_prefix = 'unice_rank_model_v7_1'
batch_size = 128
...
def train_process(data_path, cur_time, model_dir):
    ...
    # 过滤没有 user_id 用户
    raw_sample.dropna(subset=['id'], inplace=True)
    print("过滤 uid 为空的情况", raw_sample.shape)
    # 过滤：
...
if __name__ == '__main__':
    ...
    for i in range(0, date_num):
        ...
        model_dir = '../model/{}_{}'.format(model_path_prefix, date)
        if os.path.exists(data_path):
            if os.path.exists(model_dir):
                print("model path {} exist!".format(model_dir))
                if not args.overwrite:
                    # 原则上不允许 overwrite, 可手动清除 model
