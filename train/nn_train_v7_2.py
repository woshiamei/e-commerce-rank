...
from feature_handle_v4 import features_extract_2, fea_inputs_2
...
parse.add_argument('--date_num', type=int, default=60)
parse.add_argument('--cpu_num', type=int, default=10)
...
model_path_prefix = 'unice_rank_model_v7_2'
batch_size = 128
...
def train_process(data_path, cur_time, model_dir):
    ...
    # 过滤没有 user_id 用户
    raw_sample.dropna(subset=['fullvisitorid'], inplace=True)
    print("过滤 vid 为空的情况", raw_sample.shape)
    # 过滤：
    ...
    for fea in ['fullvisitorid', 'productsku', 'lace_size', 'pro_type', 'product_color', 'product_texture', 'wigtype']:
    ...
    data['target'] = np.where(data['action'] > 0, 1, 0)
    data = data.drop(columns=['action', 'random', 'p_view', 'p_click', 'is_addtocart', 'is_order', 'email', 'date'])
    gc.collect()
    ...
    if not os.path.exists(model_dir):
        ...
        # model
        feature_inputs = fea_inputs_2()
        feature_columns = features_extract_2()
        feature_layer = tf.keras.layers.DenseFeatures(feature_columns)
        inputs = feature_layer(feature_inputs)
        # inputs = tf.keras.layers.DenseFeatures(feature_columns)(feature_inputs)

        output = tf.keras.layers.Dense(128, name='bottom_1', activation=tf.nn.relu)(inputs)
        ...
        model = tf.keras.Model(inputs=feature_inputs, outputs=[pctr])
