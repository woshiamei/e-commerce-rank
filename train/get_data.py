...
sys.path.append("/root/reco/rank/")
...
start_time = (datetime.datetime.now() - datetime.timedelta(days=days)).strftime("%Y-%m-%d").replace("-", "")
         ...
         from (
          ...
          from (
                ...
                from (
                    ...
                    select fullvisitorid, `date`, userid, email, entity_id
                    from (
                        select fullvisitorid, `date`, userid, email, entity_id
                            ...
                            and shop_mark = 'unicemall'
                            and platform = '自营站'
                            -- order by fullvisitorid, email, `date`,userid, entity_id
                    )tt1 where rank = 1
