...
parse.add_argument('--date', type=str, default='20220820')
parse.add_argument('--jobs_index', type=int, default=1)
parse.add_argument('--jobs_num', type=int, default=2)
parse.add_argument('--per_vids_num', type=int, default=10)
parse.add_argument('--per_insert_num', type=int, default=100)
parse.add_argument('--model_path_prefix', type=str, default='unice_rank_model_v7_2')
args = parse.parse_args()

# 仅处理新用户（没有登录）单独进行预估处理
# 单目标处理
...
def common_predict(data, num):
	...
	pred_res_df[['vid', 'pid']] = data[['fullvisitorid', 'productsku']]
	pred_res_df['pctr'] = res
	# pred_res_df[['pctr']] = pd.DataFrame(np.concatenate([res[0]], axis=1))
	# 将 多个label 拼在一起

	pred_res_df.columns = ['vid', 'pid', 'pctr']
	...
	pred_res_df['score'] = pred_res_df.apply(lambda x: 1.0 / (x.rank_pctr + smooth), axis=1)
	...
	pred_res_df.drop(['rank_pctr'], axis=1, inplace=True)
	...
	print("upsert sql done!")

	if topN_2 < topN_1:
		pred_res_df = pred_res_df.groupby(by='vid').apply(lambda x: x.sort_values('score', ascending=False))
		pred_res_df = pred_res_df.reset_index(drop=True).groupby(by='vid').head(topN_2)
	...
	for i in pred_res_df.itertuples():
		...
		date = cur_time
		data_list.append((vid, '-1', pid, pctr, '0.0', date))
		if num % per_insert_num == 0 and num != 0:
			t22 = time.time()
...
def rank_strategy(vid, cand_df):
	...
	if len(pids_papo) == papo_num:
		...
		for i in range(papo_num):
			...
			try:
				scores.append(cand_df.iloc[index, 3])
			except:
...
def papo():
	...
	print("papo pids : {}".format(pids_papo))

def sample_struct_new(vids_to_uids_df):
	print("sample_struct 线程数： {}".format(jobs_index))
	global hdb
	hdb = hive_connect()
	...
	vids_to_uids_df_index = vids_to_uids_df.iloc[row_start: row_end, :]
	del vids_to_uids_df

	print("本次处理的行数： {} -- {}".format(row_start, row_end))
	print("本次需要处理的 vids num: {}".format(vids_to_uids_df_index.shape[0]))
	# print("uids num: {}".format(uids_feas_df.shape[0]))

	# 进行数据预处理
	# user_info, pids_feas_df = pre_handle(vids_to_uids_df, uids_feas_df, pids_xtr_df)
	user_info, pids_feas_df = pre_handle_new(vids_to_uids_df_index, pids_xtr_df)
	...
	for vid in vids_set:
		...
		if i % per_vids_num == 0 and i > 0:
			...
			pred_data.columns = ['fullvisitorid',
			                     # 'id', 'gender', 'age_range', 'country', 'region', 'city', 'register_channel',
...
def predict_run_new():
	...
	print("before drop uid is not nan num : {}".format(vids_to_uids_df.shape[0]))
	vids_to_uids_df = vids_to_uids_df[vids_to_uids_df['id'].isna()]
	print("after drop uid is not nan num : {}".format(vids_to_uids_df.shape[0]))
	vids_to_uids_df.drop(['id'], axis=1, inplace=True)
	...
	print("爬坡产品： {}".format(pid_no_xtr))
	# global pid_no_xtr_df
	...
	sample_struct_new(vids_to_uids_df)
	# 改为手动

	# 在这里进行多线程
	# repetitions = jobs_num
	# Parallel(n_jobs=repetitions)(delayed(sample_struct_new)(vids_to_uids_df, pids_feas_df,  i) for i in range(repetitions))
	# Parallel(n_jobs=repetitions)(delayed(sample_struct_new)(i) for i in range(repetitions))
	# Parallel(n_jobs=repetitions, backend="threading")(delayed(sample_struct_new)(i) for i in range(repetitions))
...
if __name__ == '__main__':
	...
	if os.path.exists(file_vids_to_uids) and os.path.exists(file_vids_pids_sum) \
		...
		print("cur_time: {} data is ready!".format(cur_time))
	else:
		print("cur_time: {} data is not ready!".format(cur_time))
		print("exit!")
		exit()
	...
	tf.config.threading.set_inter_op_parallelism_threads(thread_num)
	model = tf.keras.models.load_model(model_dir)
