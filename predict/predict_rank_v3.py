...
parse.add_argument('--jobs_num', type=int, default=1)
...
# if args.date != "":
# 	cur_time = datetime.datetime.strptime(args.date, '%Y%m%d')
...
def common_predict(data, num):
	...
	for vid in vids:
		...
		if num % (per_insert_num // 10) == 0 and num != 0:
			t24 = time.time()
...
def sample_struct(num):
	...
	# 进行数据预处理
	user_info, pids_feas_df = pre_handle(vids_to_uids_df_index, uids_feas_df, pids_xtr_df)
	del vids_to_uids_df
	del uids_feas_df
	del pids_xtr_df
...
def predict_run():
	...
	print("before drop uid is nan num : {}".format(vids_to_uids_df.shape[0]))
	vids_to_uids_df.dropna(subset=['id'], inplace=True)
	print("after drop uid is nan num : {}".format(vids_to_uids_df.shape[0]))
