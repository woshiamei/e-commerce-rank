...
parse.add_argument('--date', type=str, default='20220810')
parse.add_argument('--row_index', type=int, default=1)
parse.add_argument('--row_num', type=int, default=10)
parse.add_argument('--per_vids_num', type=int, default=10)
parse.add_argument('--per_insert_num', type=int, default=100)
parse.add_argument('--model_path_prefix', type=str, default='unice_rank_model_v6_2')
...
def rank_strategy(vid, cand_df):
	...
	# 排序
	res = list(sorted(dict_pid_score.keys(), key=lambda k: k[1], reverse=True))[:topN]
	return res
...
def predict_run():
	...
	# 分批次进行处理
	# 该进程的处理的行数
	row_start = int(vids_to_uids_df.shape[0] * ((row_index - 1) / row_num))
	row_end = int(vids_to_uids_df.shape[0] * (row_index / row_num))
	vids_to_uids_df = vids_to_uids_df.loc[row_start: row_end].copy()
	...
	t2 = time.time()
	print("数据预处理耗时： {}".format(t2 - t1))

	vids_set = user_info['fullvisitorid'].unique()

	print('pids: {}'.format(pids_set))
	# sep = ','
	tmp_data = []
	i = 0
	j = 0
	for vid in vids_set:
		...
		if i % per_vids_num == 0 and i > 0:
			...
			pred_data.columns = ['fullvisitorid', 'id', 'email',
	...
	print('size not match num: {}'.format(j))
	t3 = time.time()
	print("row_index: {} ， 整个预估过程耗时： {}".format(row_index, t3 - t1))
...
if __name__ == '__main__':
	...
	t1 = time.time()
	print("row_index: {}, row_num: {}".format(row_index, row_num))
